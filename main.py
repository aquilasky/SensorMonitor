...
class SensorMonitorApp(ctk.CTk):
    ...
    def save_average_data(self):
        ...
        try:
            ...
            self.log_message(f"已保存平均值: 温度{avg_temp:.1f}°C, 湿度{avg_humidity:.1f}%, CO2{avg_co2}ppm")
            
        except Exception as e:
            self.log_message(f"保存错误: {str(e)}")
            
        self.data_buffer = {"temperature": [], "humidity": [], "co2": []}
    ...
    def open_settings(self):
        ...
        ctk.CTkLabel(settings_win, text="数据保存间隔 (秒):").pack(pady=(20, 5))
        
        interval_var = ctk.StringVar(value=str(self.save_interval))
        interval_entry = ctk.CTkEntry(settings_win, textvariable=interval_var, width=150)
